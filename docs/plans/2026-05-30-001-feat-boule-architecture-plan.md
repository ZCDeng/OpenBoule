...
> **U0 执行回填（2026-05-30，真跑 spike，见 `spikes/RESULTS.md`）**：U0 五条退出条件 5/5 PASS（含 KTD-17 6 事件映射）。真跑暴露 5 处事实已回填本 plan——(a) 真值源 repo 确认私有 → `GITHUB_TOKEN` 由"防限流"升级为**强制**（KTD-2/KTD-20/U2）；(b) TS SDK 实际入口是 `query({includePartialMessages})`，非 Python 的 `ClaudeSDKClient` 类（KTD-2/KTD-17/U3）；(c) headless worker 需真 `ANTHROPIC_API_KEY`，本轮用 CLI 订阅会话跑通（`apiKeySource=none`），`messages-api` 裸 key fallback 未验 → Open Q 13；(d) 单 role 调用 ≈$0.16–0.50、prompt caching 命中显著（KTD-22/U3）；(e) `thinking_delta` live 未触发仅 fixture 证（U3 contract 测试注明）。Agent SDK headless 风险由"高"降"低"。
...
| KTD-2 | Role 执行器 | **Agent SDK（按需 spawn）** | "按需 spawn"（phase 开始时连、结束后释放）vs "keep-warm 连接池"：选前者，Boule 是顾问工具不是高并发 SaaS，phase 间有人审批间隔，keep-warm 浪费资源。**U0 修正命名**：`ClaudeSDKClient` 是 Python SDK 叫法；TS SDK（`@anthropic-ai/claude-agent-sdk`）实际入口是 `query({ options: { systemPrompt, allowedTools, includePartialMessages } })`，无 `ClaudeSDKClient` 类——本 plan 后文 `ClaudeSDKClient` 均指此 `query()` 路径。**U0 修正 auth**：headless worker 需真 `ANTHROPIC_API_KEY`（`apiKeySource=user`）；本机 spike 用 CLI 订阅会话跑通（`apiKeySource=none`）仅作开发验证 |
...
| KTD-22 | 成本可观测 | **v1：自建三层 cost 表为结算真值源；langfuse trace 推迟** | 多用户要结算/配额/账单，不能像 OD 把金额外包给 langfuse——自建 cost 表（run/phase/job）是 v1 唯一真值源。langfuse 旁路**移到 Deferred**（plan Open Q 本就建议默认关、cost 表先行，且零 v1 消费者）。**采纳前置条件**：langfuse `generation` span 默认捕获 prompt/completion，会把客户机密（宇润/正治 等真实交付）发第三方——启用前必须默认 `mask_input/mask_output`（只传 token 数 / model / 延迟），全量 prompt 日志另设独立 env flag。其 `trace → span → generation` 映射 `run → phase → job` 的可视化价值留待 fan-out 调试确有需要时再做。**U0 实测量级**：单 role 调用 ≈$0.16–0.50（input 被 ~13.8k 字符 role system prompt 主导），prompt caching 命中显著（spike1 `cache_read=29k`）——cost 表 day-1 即需要，且 U3 应显式利用 prompt caching 压低重复 role prompt 的 input 成本 |
...
**执行记录（2026-05-30，真跑，详见 `spikes/RESULTS.md`）：✅ 5/5 PASS，未触发回退。**
- ✅ Agent SDK headless 跑通真实 `information-architect.md`（@`1e29e50`），产出合规 test axis
- ✅ tool 路径：`allowedTools` 声明被接受，agent 真实调用 Bash 并返回（`tool_use`/`tool_result` live）；WebSearch 同机制（本轮未单独 live 调）
- ✅ 并发隔离：3 role 并发 9.2s，usage 按 `jobId` 独立归账，Σ per-job = 总计
- ✅ executor 消费固化 truth snapshot：live 拉**私有** repo 固化 `{commit_sha, manifest, truth_digest}`，按 hash 校验读快照；上游漂移→digest 变而在途快照内容不变
- ✅ `query()` → 6 类归一化事件映射成立：6/6 均有真实 SDK 来源（5 live + `thinking_delta` fixture 兜底）。映射为逐条消息纯函数（spike `normalize.ts`），是 U3 `runtime-contract.test.ts` 雏形
- **遗留待 U3 补验**：`messages-api` 裸 key fallback 路径未验（按 auth 决策推迟，见 Open Q 13）；`thinking_delta` 的 live 覆盖取决于 thinking 是否分块下发
...
- **GitHub token 最小权限**：`GITHUB_TOKEN` 必须是 fine-grained PAT，**只读、只限 skills 单仓 Contents** 权限（默认宽权限 PAT 一旦服务环境泄露＝攻击者可改真值源）。token 走环境密钥不入库；启动时 dry-run 校验，缺失/无效则 fail loud。**U0 确认**：真值源 repo `ZCDeng/consulting-team` 是**私有**的——token 因此是**强制**（非仅防限流），`raw.githubusercontent.com` 读私有文件必须带 `Authorization: Bearer <token>`（U0 已验 HTTP 200）。
...
- **runtime-def 抽象（KTD-17，借鉴 open-design `runtimes/`）**：抽象边界划在"产出归一化事件流"，不划在"如何调用后端"。定义 `BouleRoleRuntime.run(roleCtx): AsyncIterable<NormalizedEvent>`，事件统一为 6 类：`text_delta` / `thinking_delta` / `tool_use` / `tool_result` / `usage` / `status`。两个实现：
  - `claude-sdk`：按需调 `query({ options: { systemPrompt, allowedTools, includePartialMessages: true, maxTurns } })`（**U0 修正**：TS SDK 入口是 `query()` 非 `ClaudeSDKClient` 类；`includePartialMessages: true` 是出 `text_delta`/`thinking_delta` 的**必要**开关），迭代其消息流（`system/init` → `stream_event` → `assistant`/`user` → `result`）归一成 6 类事件。**利用 prompt caching**：role system prompt（~13.8k 字）跨同 role 调用复用，U0 实测 `cache_read` 命中显著，可大幅压 input 成本
...
- Contract: `claude-sdk` 与 `messages-api` 对同一 `roleCtx` 输入产出**经归一化后语义等价**的事件流（最终状态一致 + 关键事件类型齐全；不要求逐字节相同序列——底层流分块不同）。**U0 已证** `claude-sdk` 侧 6 类事件映射（live + fixture）；`messages-api` 侧裸 key 路径**待 U3 有 key 时补验**（U0 按 auth 决策未跑）
- Contract（thinking_delta 覆盖注记）：`thinking_delta` 的归一化结构 U0 已证（fixture），但 **live 是否出 delta 取决于 thinking 是否开启/分块下发**；contract 测试用 fixture 锁结构，不强求每次 live run 都出 `thinking_delta`
...
### 从 2026-05-30 U0 spike 沉淀
13. **`messages-api` 裸 key fallback 未验**（U0 按 auth 决策推迟）：U0 仅验 `claude-sdk` 路径（CLI 订阅会话）。U3 实现 `messages-api` runtime 时须用真 `ANTHROPIC_API_KEY` 跑 runtime-contract 对照，确认两 runtime「经归一化后语义等价」；在此之前 fallback 路径是**未验证假设**，不可声称可用
...
| 风险 | 严重度 | 缓解 |
|---|---|---|
| Agent SDK `query()` 在服务端 headless 环境不稳定 | ~~高~~ → **低（U0 已降险）** | **U0 已验** headless `query()` 跑通真实 role + tool 调用 + 6 事件映射（CLI 订阅会话）。残留：生产 worker 换真 `ANTHROPIC_API_KEY` 跑通（`apiKeySource=user`）；fallback 到 `messages-api` runtime（Open Q 13 待验） |
