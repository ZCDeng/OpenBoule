...
- **review deferred — P2 #6-10 已清（`fix/code-review-p2`）**：#6 search_research 上限 20+并行拉取；#7 link_mode 转 pgEnum（迁移 0004，text→enum 带 USING cast）；#8 verifyApiKey 单条 UPDATE…RETURNING 原子化；#9 import 行数上限（500 workflow / 1000 artifact）；#10 POST artifact 响应 artifactId→id（与 GET 一致）。+2 测（行数上限/search_research），api 180 绿，迁移 0004 应用。

## 2026-06-01 F4 Web 工作流可审计 + 项目 references + 运行时设置

直接合 main（无 PR），随后单线程代码审核 + 实跑验证 + 重构收尾。

| 单元 | 状态 | 要点 |
|---|---|---|
| 实时可审计 | ✅ | `agent-runner` 的 `normalizeAgentProgressEvent` 在写 `workflow_events` 表**之前**就把 `thinking_delta` / `text_delta` 滤成 null，只持久化 tool_use / tool_result / usage / status；前端 `workflow-events.ts` 二次过滤做防御。workflow-status 仅状态变更事件才 refetch，防请求风暴。 |
| 项目 references | ✅ | 输入材料独立通道（映射 Skill `sources/`，不复用 artifact/submit_artifact）+ workflow 启动冻结快照。校验 UUID/256KB/20 条/filename 长度，全程参数化 SQL。迁移 0005。 |
| 运行时设置 | ✅ | `/api/settings/runtime` 只读总览：不暴露密钥；CLI/API 调用方式服务端决定（首版不给 Web 切换）。`rejectApiKeyAuth` 只允许会话访问。 |
| 文档工作台 | ✅ | Editor 可编辑状态 + VersionHistory 前端打通。 |

- **审计安全边界在后端持久化层强制**，不依赖前端——直连 SSE 也拿不到思考过程。对应 commit message 约束「Agent events must not expose thinking process or raw high-volume model text」。
- **审核非阻塞观察 → 重构收尾**：DELETE reference 路由内联 SQL 下沉 service（`3d4f43e`，抽 `deleteProjectReference` 返回 boolean，清死 import）；沉淀 `docs/solutions` 首条（`33b174e`，确立 frontmatter 约定）。
- 验证：api typecheck 0 + 184 测、web 32 测全绿（含 `filters thinking_delta` 约束测试）；迁移 0005 应用。commits `9fffd3c` / `3d4f43e` / `33b174e` / `da5bd92`。
- **#5 已清（plan 003 completed）**：`authenticate` → `makeAuthenticate(db)` 工厂；11 文件各加 `const authenticate = makeAuthenticate(db)`（调用点零改动，局部遮蔽）；删 singletonDb import（tsc 兜底漏迁移）。根因回归测：注入查不到 key 的 db → 真 key 也 401（证明走注入 db 非单例）。验证：singletonDb 0 引用 / makeAuthenticate 11 文件 / api 181 绿。**code-review deferred 全清零**。

## 2026-06-05 F5 Phase 3.5 评审合议（跟随 consulting-team v2.5，10 阶段/7+3）

Plan：`docs/plans/2026-06-05-001-feat-phase3_5-review-panel-plan.md`。触发：上游 consulting-team
`2cfdabb` 在 Phase 3(综合)与 Phase 4(三筛)之间新增**强制 Phase 3.5 实质评审合议**（5视角→三票→
ship/revise/rework）。Boule 作为其方法论 Web 化产品全站跟随，9 阶段→10 阶段。commits `80a1d21`(feat)
+ `d0fe38f`(merge)，已推 origin/main。

**关键决策（前后端必须一起改，非纯文案）**：`PHASE_LABELS`(前端 `lib/phases.ts`)与 `PHASE_IDS`
(后端 `workflow/state.ts`)是**双职责共享真值源**——既驱动 Landing/Methodology 营销展示，也驱动
Timeline/Workspace/CostChart/MethodologyGraph 实时工作台状态。只改文案不改引擎，前端会出现后端永不
激活的幽灵阶段。故 Phase 3.5 落进真正可执行的 BullMQ 工作流。

| 层 | 改动 | 要点 |
|---|---|---|
| 后端引擎 | `state.ts` / `phases/index.ts` / `engine.ts` / `agent-runner.ts` / `approvals.ts` | PHASE_IDS 插 `phase3_5_review` + 新 `panel` 形态(类比 serial：一个 job 跑 N 视角 + 合议)；`evaluateReviewPanel` 纯函数三分支裁决(ship/revise/rework)；新 `rework` 决策固定退回 `phase3_synthesis`(复用 augment「人确认 + 新 attempt」模式，**不自动循环**——人就是循环上限)；`runReviewPanel` + `processPanel` + `engine.rework()`；reviewer-*→source-verifier 复用对抗验证 role(视角经 task 透传)；approvals 决策表加 `rework` 路由 |
| lineage | `services/lineage.ts`（无改） | `downstreamPhases` 是 index-based 按 PHASE_IDS 切片，插入 phase id 自动把 3.5 纳入 phase3 下游 |
| 前端 | `lib/phases.ts` / `Methodology.tsx` / `Landing.tsx` / `Login.tsx` | phases.ts 加一行 → 所有泛型消费方(时间线/工作台/成本图/编排图)自动跟随；展示页文案与数字 9→10、7+2→7+3 |
| 文档/图 | `README.md` / `boule-pipeline.svg`+`.png` / `boule-pipeline-brutalist.svg`+`.png` / `_design/landing-brutalist-demo.html` | 两张流水线 SVG 手工重排版插 Phase 3.5 节点(主图蓝→teal panel；brutalist 竖栈 +87px 长画布、shift P4/5/6、reroute 连接线)，`rsvg-convert` 重出 PNG 并肉眼验图；设计稿同步 |

- **验证**：API **217 pass / 0 fail / 2 skip**(live-SDK 无凭证)；engine + HTTP e2e 真走通 10 阶段经 panel；
  web 54 测 + 生产 build；两端 tsc 干净；全仓 `9 阶段/7+2/九阶段/STAGES` 零残留。
- **留痕（deferred，见 features.json F5）**：reviewer 的 composite/mustFix 为启发占位（同 editor 的 0.85），
  真实评审质量分 + debates 解析待接入；前端暂无 readiness 专用 checkpoint UI（走通用审批 + rework 路由）。
- **未做**：Boule 引擎实现为内建 `panel` 形态，**不**直接调 consulting-team 那个 `consulting-review-panel.js`
  workflow 脚本（Boule 引擎自有 BullMQ 编排，源不同实现）。

---

## 2026-06-14 — 跟进 consulting-team 上游 Jun 13 更新（仅展示层）

上游 `consulting-team` 在 Jun 5（Boule 跟进 Phase 3.5）之后于 **Jun 13** 有 3 个方法论提交：
`b82fa55`（Phase 4 editor 新增 **filter-3b 信息有效性审查**，只标记不 block；Phase 5 designer 新增
**第 5 交互交付轨** effective-html）、`0a7414d`（核心**纪律 19：客户交付物=机密，绝不上公网托管** +
Step 4.6 交互件 QR 回贴 deck 封底）、`ddc3b2b`（`interactive/` 归档目录）。

**评估**：三项均为 Phase 4/5 **角色级细化**，不新增阶段、不动 10 阶段状态机。按双职责共享源纪律
（改阶段才前后端同步），核心引擎无需跟。本轮只同步**方法论展示层**文案：

| 文件 | 改动 |
|---|---|
| `lib/phases.ts` | Phase 4 note +「含信息有效性审查」；Phase 5 note +「可选交互件」 |
| `pages/Methodology.tsx` | Phase 4 output「三筛 + 信息有效性」；Phase 5 output +「可选交互件」 |
| `README.md` 角色表 | 审稿编辑 +「（含信息有效性审查）」；设计师 +「可选交互件」 |
| `pages/Landing.tsx` | 角色卡 R4/R5 同步同上 |

**未做**：纪律 19（机密不上公网）是运营纪律、非流水线步骤，方法论 UI 无对应区块，未塞进界面；
留作 Boule 引擎将来生成/托管客户交付物时的产品逻辑红线（与产品官网 Landing 公开部署不冲突）。

---

## 2026-06-14（下午）— 落地 Phase 5 第 5 交互交付轨（Step 4.5）

按 `docs/plans/2026-06-14-001-feat-phase5-interactive-track-plan.md` 全量执行。**不动 10 阶段状态机**——
交互件是 Phase 5 内 opt-in 的第二个 artifact（`type:"interactive"`，唯一索引含 type，与 phase5_delivery 零冲突）。

- 后端：`state.ts` 三纯函数（defaultInteractiveKind / lintSelfContained）+ `runInteractiveTrack`（designer 复用，
  role `interactive-<kind>` 前缀 + 运行时注入「无 fs/技能直接吐 HTML」覆盖）+ engine `maybeRunInteractive`（读
  checkpoint_data.interactiveTrack，lint 软门，失败不阻断标准交付）+ `POST /interactive-track` opt-in 端点。
- 前端：`InteractiveTrackPicker`（phase4 checkpoint 可决策时显示）+ Workspace iframe 屏幕预览 + DocumentList「不进 PDF」标 + 三事件友好文案。
