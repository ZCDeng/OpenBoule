...
- **#5 已排独立 plan**：`docs/plans/2026-05-31-003-refactor-authenticate-db-injection-plan.md`（工厂 `makeAuthenticate(db)` 对齐 `requireProjectRole`，11 文件各加一行 + 删单例 import + 1 根因回归测；纯重构零行为变更）。待执行。
