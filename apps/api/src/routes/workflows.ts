...
import { freezeWorkflowReferences, loadProjectReferencesPartitioned, validateReferenceIds } from "../services/references.ts";
...
export function registerWorkflowRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const referenceSelection = await loadProjectReferencesPartitioned(db, projectId!, refsInput.ids);
      const references = referenceSelection.loaded;
      ...
      return reply.code(201).send({
        ...
        skippedReferences: referenceSelection.skipped,
      });
    },
  );
