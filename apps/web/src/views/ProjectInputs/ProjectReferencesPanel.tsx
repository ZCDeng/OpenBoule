...
export function ProjectReferencesPanel({ projectId, selectedIds, onSelectedIdsChange }: { projectId: string; selectedIds: string[]; onSelectedIdsChange: (ids: string[]) => void }) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between gap-4 pb-4" style={{ borderBottom: "1px solid var(--md-outline-variant)" }}>
        <div><h2 className="font-[var(--boule-disp)] text-2xl font-semibold tracking-[-0.01em]">项目材料</h2><p className="mt-2 text-sm text-[var(--md-on-surface-variant)]">上传客户参考/原始材料。启动任务时只存档勾选的材料。</p><p className="mt-1 text-xs text-[var(--boule-orange)]">扫描件或混合扫描文档会发送至 Anthropic/Claude 抽取文本；数字文档优先本地解析。</p></div>
