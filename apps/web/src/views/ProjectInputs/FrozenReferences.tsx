    ...
    <Panel>
      ...
      <div className="boule-panel-body">
        <div className="border-2 border-black">
          {references.map((ref) => { const open = openId === ref.id; return <div key={ref.id} className="border-t-2 border-black first:border-t-0"><button onClick={() => setOpenId(open ? null : ref.id)} className="flex w-full items-center justify-between gap-3 px-3 py-3 text-left"><span className="min-w-0 truncate font-[var(--boule-disp)] font-black tracking-[-0.02em]">{ref.filename}</span><Badge>{humanBytes(ref.sizeBytes)}</Badge></button>{open && <pre className="max-h-64 overflow-auto whitespace-pre-wrap border-t-2 border-black p-3 font-[var(--boule-mono)] text-xs">{ref.bodySnapshot}</pre>}</div>; })}
        </div>
      </div>
    </Panel>
  );
