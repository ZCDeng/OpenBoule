  ...
  return (
    <Panel>
      ...
      <div className="boule-panel-body">
        <div className="overflow-hidden rounded-[var(--md-shape-md)] border border-[var(--md-outline-variant)]">
          {references.map((ref) => { const open = openId === ref.id; return <div key={ref.id} className="border-t border-[var(--md-outline-variant)] first:border-t-0"><button onClick={() => setOpenId(open ? null : ref.id)} className="flex w-full items-center justify-between gap-3 px-3 py-3 text-left"><span className="min-w-0 truncate font-semibold tracking-[-0.01em] text-[var(--md-on-surface)]">{ref.filename}</span><Badge>{humanBytes(ref.sizeBytes)}</Badge></button>{open && <pre className="max-h-64 overflow-auto whitespace-pre-wrap border-t border-[var(--md-outline-variant)] p-3 font-[var(--boule-mono)] text-xs text-[var(--md-on-surface)]">{ref.bodySnapshot}</pre>}</div>; })}
        </div>
      </div>
    </Panel>
  );
