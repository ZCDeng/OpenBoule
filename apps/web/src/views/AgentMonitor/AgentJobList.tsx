...
export function AgentJobList({ jobs }: { jobs: AgentJobRow[] }) {
  ...
  if (jobs.length === 0) return <p className="border-2 border-dashed border-[var(--app-fg)] p-4 font-[var(--boule-mono)] text-xs uppercase tracking-[0.1em] text-[var(--boule-muted)]">启动一个任务后在此查看执行明细。</p>;
  ...
  return (
    <div className="overflow-auto border-2 border-[var(--app-fg)]" style={{ height: VIEWPORT_H }} onScroll={(e) => setScrollTop(e.currentTarget.scrollTop)}>
      <div ref={listRef} style={{ height: jobs.length * ROW_H, position: "relative" }}>
        {visible.map((j, i) => {
          const idx = start + i;
          return (
            <div key={j.jobId ?? idx} data-agent-job-row className="flex items-center gap-3 border-b-2 border-[var(--app-fg)] px-3 text-sm" style={{ position: "absolute", top: idx * ROW_H, height: ROW_H, left: 0, right: 0 }}>
