...
export function CostChart({ data }: { data: CostBar[] }) {
  ...
  const baseY = H - 22;
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full border-2 border-black bg-[var(--boule-paper)]" role="img" aria-label={`各阶段成本，峰值 $${max.toFixed(2)}`}>
      {/* 峰值参考 + 基线，让柱高能读出实际数值 */}
      <text x={6} y={12} className="fill-[var(--boule-muted)] text-[7px] font-bold">峰值 ${max.toFixed(2)}</text>
      <line x1={0} y1={baseY} x2={W} y2={baseY} stroke="#0B0B0B" strokeWidth={1.5} />
      {data.map((d, i) => {
        ...
        return (
          <g key={d.phase}>
            ...
            <text x={cx} y={Math.max(10, barTop - 3)} textAnchor="middle" className="fill-black text-[7px] font-bold">${d.costUsd.toFixed(2)}</text>
            <text x={cx} y={H - 7} textAnchor="middle" className="fill-black text-[8px] font-bold">{phaseShort(d.phase)}</text>
          </g>
        );
      })}
    </svg>
