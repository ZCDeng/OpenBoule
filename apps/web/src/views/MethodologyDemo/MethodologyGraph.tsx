...
export function MethodologyGraph({ statusByPhase }: { statusByPhase?: Record<string, PhaseRunStatus> }) {
  ...
  return (
    <div className="relative h-[600px] border-2 border-black bg-[var(--boule-paper)] shadow-[6px_6px_0_#0B0B0B]">
      ...
      {open && <aside className="absolute right-0 top-0 h-full w-72 overflow-auto border-l-2 border-black bg-[var(--boule-paper)] p-4 shadow-[-5px_0_0_#0B0B0B]"><div className="flex items-start justify-between gap-3"><div><Badge tone="blue">阶段</Badge><h3 className="mt-3 font-[var(--boule-disp)] text-2xl font-black tracking-[-0.03em]">{open.label}</h3></div><button onClick={() => setOpenId(null)} className="border-2 border-black px-2 py-1 font-[var(--boule-mono)] text-xs">✕</button></div><p className="mt-4 text-sm text-[#33332e]">{open.note}</p></aside>}
    </div>
  );
}
