  ...
  return (
    <div ref={workspaceRef} className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[240px_1fr] xl:grid-cols-[240px_1fr_260px]">
        ...
        <section className="space-y-3">{!selectedId ? <EmptyState title="选择左侧文档开始编辑" /> : selected.isLoading ? <Skeleton rows={6} /> : selected.data ? (<>
          <ExportBar name={`${phaseLabel(selected.data.phase)}-v${selected.data.version}`} body={selected.data.body ?? ""} isHtml={selected.data.type === "interactive"} />
          {selected.data.type === "interactive" ? <InteractivePreview body={selected.data.body ?? ""} /> : <Editor key={selected.data.id} artifactId={selected.data.id} initialBody={selected.data.body ?? ""} meta={{ phase: selected.data.phase, phaseLabel: phaseLabel(selected.data.phase), type: selected.data.type, version: selected.data.version, status: selected.data.status, stale: selected.data.stale || stalePhases.has(selected.data.phase) }} readOnly={readOnlyHistory} onSaved={handleSaved} />}
        </>) : null}</section>
