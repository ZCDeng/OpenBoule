  ...
  return (
    <div className="space-y-5">
      ...
      {links.length === 0 ? <EmptyState title="暂无分享链接" hint="创建分享链接，客户无需登录即可只读查看。" /> : (
        <ul className="border-2 border-[var(--app-fg)] text-sm shadow-[5px_5px_0_var(--app-fg)]">
          {links.map((l) => <li key={l.token} className="flex items-center gap-3 border-t-2 border-[var(--app-fg)] px-4 py-3 first:border-t-0"><code className="flex-1 truncate font-[var(--boule-mono)] text-xs">{l.url}</code><Badge>有效期至 {new Date(l.expiry).toLocaleDateString("zh-CN")}</Badge><Button variant="secondary" onClick={() => void navigator.clipboard?.writeText(`${location.origin}${l.url}`)}>复制</Button></li>)}
        </ul>
      )}
    </div>
  );
