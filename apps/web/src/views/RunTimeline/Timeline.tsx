...
export function Timeline(props: TimelineProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <ol className="space-y-4 border-l border-[var(--md-outline-variant)] pl-6">
        {PHASE_LABELS.map((p, i) => {
          ...
          return (
            <li key={p.id} className="relative">
              <span className="absolute -left-[37px] top-4 grid h-7 w-7 place-items-center rounded-[var(--md-shape-full)] border border-[var(--md-outline-variant)] bg-[var(--md-surface)] text-[11px] font-medium text-[var(--md-on-surface-variant)]">{String(i + 1).padStart(2, "0")}</span>
